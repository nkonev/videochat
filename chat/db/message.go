...
func hasUnreadMessagesBatchByChatIdsCommon(ctx context.Context, co CommonOperations, chatIds []int64, userId int64) (map[int64]bool, error) {
	...
	// left join and coalesce in order to process a cornercase when for an user there is still no record in message_read
	var q = `
		select maxes.chat_id, max_message_id > coalesce(last_message_id, 0) as has_new_message_for_user from
		(select chat_id, max(id) as max_message_id from message where chat_id = ANY($1) group by chat_id order by chat_id) maxes 
		left join (SELECT chat_id, last_message_id FROM message_read WHERE user_id = $2 AND chat_id = ANY($1) order by chat_id) reads
...
func hasUnreadMessagesCommon(ctx context.Context, co CommonOperations, userId int64) (bool, error) {
	...
	const size = 10 * utils.DefaultSize
