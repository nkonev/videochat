...
func (h *FilesService) GetListFilesInFileItem(
	...
	res, errOuter := db.TransactWithResult(c, h.dba, func(tx *db.Tx) (*getListFilesInFileItemResult, error) {
		metadatas, err := db.GetList(c, tx, chatId, fileItemUuid, filterObj, true, size, offset)
		if err != nil {
...
func SerializeMetadataSimple(userId int64, correlationId *string, isConferenceRecording *bool, isUserMessageRecording *bool, timestamp int64) map[string]string {
	...
	userMetadata[timestampKey] = utils.Int64ToString(timestamp)
	return userMetadata
}
...
func DeserializeMetadata(userMetadata minio.StringMap, hasAmzPrefix bool) (int64, string, int64, error) {
	...
	var timestamp int64
	timestampString, ok := userMetadata[prefix+strings.Title(timestampKey)]
	if ok {
...
func TimestampKey(hasAmzPrefix bool) string {
	var prefix = ""
	if hasAmzPrefix {
		prefix = xAmzMetaPrefix
	}
	return prefix + strings.Title(timestampKey)
}
