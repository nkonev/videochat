...
func (db *DB) GetNotificationSettings(userId int64) (*dto.NotificationSettings, error) {
	row := db.QueryRow(`select mentions_enabled, missed_calls_enabled from notification_settings where user_id = $1`, userId)
	if row.Err() != nil {
		return nil, row.Err()
	}
	var result = new(dto.NotificationSettings)
	err := row.Scan(&result.MentionsEnabled, &result.MissedCallsEnabled)
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (db *DB) PutNotificationSettings(userId int64, to *dto.NotificationSettings) error {
	if _, err := db.Exec(`update notification_settings set mentions_enabled = $2, missed_calls_enabled = $3 where user_id = $1`, userId, to.MentionsEnabled, to.MissedCallsEnabled); err != nil {
