...
import {
  defaultAudioMute,
  getWebsocketUrlPrefix, isFireFox, isMobileBrowser,
  isMobileFireFox
} from "@/utils";
import {
  ...
  methods: {
    ...
    async createLocalMediaTracks(videoId, audioId, isScreen) {
      ...
      try {
        ...
        console.debug("isFirefox = ", isFirefox, " in case Firefox simulcast for video tracks will be disabled");
        for (const track of tracks) {
          const normalizedIsScreen = !!isScreen;
          const trackName = "track_" + track.kind + "__screen_" + normalizedIsScreen + "_" + this.getNewId();
          const simulcast = !isFirefox && (normalizedIsScreen ? this.screenSimulcast : this.videoSimulcast);
